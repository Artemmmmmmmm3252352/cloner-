...
const App: React.FC = () => {
      ...
      pages: [
        {
          ...
          title: 'Welcome Hub',
          icon: '👋',
          coverStyle: 'bg-gradient-to-r from-[#FFDEE9] to-[#B5FFFC]',
          blocks: [
            { id: generateId(), type: 'h1', content: 'Welcome to your workspace' },
            { id: generateId(), type: 'text', content: 'This is a demo of the Surgical Alabaster workspace.' },
            { id: generateId(), type: 'bullet', content: 'Clean aesthetics' },
            { id: generateId(), type: 'bullet', content: 'Fast performance' },
          ],
          comments: [],
          updatedAt: Date.now(),
          favorite: true,
  ...
  if (isAuthChecking) return <div className="flex w-full h-screen bg-alabaster-pure items-center justify-center text-surgical-dim">Loading...</div>;
  if (!user) return <AuthPage onLogin={handleLogin} />;

  return (
    <div className="flex w-full h-screen bg-alabaster-pure text-surgical-steel font-sans overflow-hidden transition-colors duration-300">
      <Sidebar 
        currentUser={user}
      ...
      />
      <main className="flex-grow flex flex-col relative bg-gradient-to-b from-alabaster-pure to-alabaster-haze">
        <Header 
        ...
        {activePageId === 'HOME' ? (
            <HomeView pages={pages.filter(p => !p.deletedAt)} onNavigate={setActivePageIdForWorkspace} t={t} />
        ) : activePageId === 'INBOX' ? (
